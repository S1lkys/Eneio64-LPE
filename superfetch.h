...
namespace spf {
    ...
    // C++20 compatible replacement for std::expected<T, E>.
    template<typename T, typename E>
    class result {
    public:
        result(T value) : data_(std::move(value)) {}
        result(E error, std::nullptr_t) : data_(std::move(error)) {}

        static result ok(T value) { return result(std::move(value)); }
        static result err(E error) { return result(std::move(error), nullptr); }
    ...
    public:
        ...
        static result<memory_map, spf_error> current();
    ...
    inline result<memory_map, spf_error> memory_map::current() {
        if (!raise_privilege())
            return result<memory_map, spf_error>::err(spf_error::raise_privilege);

        memory_map mm = {};
        mm.ranges_ = query_memory_ranges();

        if (mm.ranges_.empty())
            return result<memory_map, spf_error>::err(spf_error::query_ranges);
        ...
        for (auto const& [base_pfn, page_count] : mm.ranges_) {
            ...
            if (!NT_SUCCESS(query_superfetch_info(
                ...
                return result<memory_map, spf_error>::err(spf_error::query_pfn);
        ...
        return result<memory_map, spf_error>::ok(std::move(mm));
    }
